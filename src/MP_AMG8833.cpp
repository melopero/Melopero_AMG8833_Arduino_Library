...
int MP_AMG8833::readByte(uint8_t registerAddress){
    ...
    if (Wire.endTransmission(false) != 0)
        return (int) MP_AMG8833_ERROR_CODE::ERROR_READING;
...
int MP_AMG8833::writeByte(uint8_t registerAddress, uint8_t value){
    Wire.beginTransmission(this->i2cAddress);
    if (Wire.endTransmission(false) != 0)
        return (int) MP_AMG8833_ERROR_CODE::ERROR_WRITING;

    Wire.beginTransmission(this->i2cAddress);
    Wire.write(registerAddress);
    Wire.write(value);
    Wire.endTransmission();
    return (int) MP_AMG8833_ERROR_CODE::NO_ERROR;
}
...
int MP_AMG8833::setMode(DEVICE_MODE mode = DEVICE_MODE::NORMAL){
    return this->writeByte(MODE_REG_ADDR, (uint8_t) mode);
}

int MP_AMG8833::updateStatus(){
...
int MP_AMG8833::clearFlags(bool clearInterrupt, bool clearPixelTempOF, bool clearThermTempOF){
...
int MP_AMG8833::setInterruptThreshold(float lowThreshold, float highThreshold, float hysteresis){
    if (lowThreshold < MIN_THRESHOLD_TEMP || lowThreshold > MAX_THRESHOLD_TEMP ||
        ...
        return (int) MP_AMG8833_ERROR_CODE::ARGUMENT_ERROR;
...
int MP_AMG8833::updateInterruptMatrix(){
...
int MP_AMG8833::updatePixelMatrix(){
    ...
    for (int regAddr = FIRST_PIXEL_REGISTER; regAddr <= LAST_PIXEL_REGISTER; regAddr += 2){
        ...
        if (msb < 0 || lsb < 0) return (int) MP_AMG8833_ERROR_CODE::ERROR_READING;
